...
import { APIService, ListProbateRecordsQuery, ModelProbateRecordCollectionFilterInput, ModelProbateRecordFilterInput, ProbateRecord, ProbateRecordCollection } from './API.service';
...
export class ProbateRecordService {
  ...
  private async getProbateRecordCollectionsByCount(filter: ModelProbateRecordCollectionFilterInput | undefined, count: number, nextToken?: string) {
    let result = await this.recordService.ListProbateRecordCollections(filter, count, nextToken);
    let newNextToken = result.nextToken;
    let recordCollections = result.items!.map((r) => r as ProbateRecordCollection);
    let ids = new Set<string>();
    ...
    while(recordCollectionCount < count && newNextToken) {
      result = await this.recordService.ListProbateRecordCollections(filter, count - recordCollections.length, nextToken);
  ...
  getProbateRecordCollections(
    ...
    pageSize: number,
    nextToken?: string
  ): Observable<{ probateRecordCollections: ProbateRecordCollection[]; nextToken: string | null }> {
    console.log('getting probate record collections');
    return from(
      this.getProbateRecordCollectionsByCount(filter, pageSize, nextToken)
    ).pipe(
      map((result: {records: ProbateRecordCollection[], nextToken: string | null | undefined}) => {
          let records = result.records;
          console.log('records returned');
          console.log(records);
          return {
